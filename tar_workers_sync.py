...
class S3SrcConfig:
    ...
    profile: Optional[str] = None   # локально: AWS profile; на EC2: обычно None
    ignore_aws_env: bool = False    # ВАЖНО для EC2: не брать AWS_ACCESS_KEY_ID из env
...
def make_s3_client_src(cfg: S3SrcConfig):
    # Если в env случайно лежат AWS_ACCESS_KEY_ID — boto3 возьмёт их вместо IAM Role
    ctx = _TempUnsetEnv(["AWS_ACCESS_KEY_ID", "AWS_SECRET_ACCESS_KEY", "AWS_SESSION_TOKEN"]) if cfg.ignore_aws_env else None
    if ctx:
        ctx.__enter__()
    try:
...
def make_s3_client_dst(cfg: S3DstConfig):
    # Timeweb: S3-compatible. Лучше path-style.
    # creds только для Timeweb — берём из env и пробрасываем явно.
    sess = boto3.Session()
    kwargs = {
...
def mark_done_batch(pg_dsn: str, ids: List[str]):
    if not ids:
        return
    c = psycopg2.connect(pg_dsn)
    try:
        with c.cursor() as cur:
            cur.execute(SQL_MARK_DONE_BATCH, (ids,))
        c.commit()
    finally:
        c.close()
...
def stream_tar_and_upload(
    ...
    *,
    ...
    returns:
      ...
      error_string
    """
    # Клиенты создаём ВНУТРИ потока — надёжно на Windows
    ...
    # буфер для DONE (ускорение: не коннектимся в PG на каждый файл)
    done_buf: List[str] = []
    ...
    try:
        ...
        for member in tf:
            ...
            # периодически ставим DONE батчем
            if done_flush_every > 0 and len(done_buf) >= done_flush_every:
                mark_done_batch(pg_dsn, done_buf)
                done_buf.clear()

        # финальный flush DONE
        if done_buf:
            mark_done_batch(pg_dsn, done_buf)
            done_buf.clear()
        ...
        # НЕ теряем прогресс uploaded; попробуем добить DONE тем, что уже успели
        try:
            if done_buf:
                mark_done_batch(pg_dsn, done_buf)
        except Exception:
        ...
        with ThreadPoolExecutor(max_workers=concurrency) as ex:
            for tar_key, lookup_to_originals in tar_plan.items():
                ...
                futures.append(ex.submit(
                    ...
                    done_flush_every=done_flush_every,
                ))
...
def main():
    ...
    ap.add_argument("--src-profile", default="", help="AWS profile for SOURCE (local). Empty -> default chain / IAM Role.")
    ap.add_argument("--src-ignore-aws-env", action="store_true",
                    help="Ignore AWS_* env vars for SOURCE so IAM Role works even if env vars exist.")

    # destination (timeweb)
    ap.add_argument("--dst-bucket", required=True)
    ap.add_argument("--dst-endpoint", required=True)
    ap.add_argument("--dst-prefix", default="pdf/")
    ap.add_argument("--dst-region", default="ru-1")
    ap.add_argument("--dst-addressing-style", default="path", choices=["path", "virtual"])

    # runtime
    ap.add_argument("--concurrency", type=int, default=6, help="parallel tar jobs INSIDE one worker process")
    ap.add_argument("--limit-rows", type=int, default=2000, help="how many arxiv_paper rows to claim per iteration")
    ...
    ap.add_argument("--stop-when-empty", action="store_true", help="exit when no NEW rows were claimed")
    ap.add_argument("--interval-sec", type=int, default=30)
    ap.add_argument("--sleep-on-empty-sec", type=float, default=5.0)
    ap.add_argument("--log-level", default="WARNING")
    ap.add_argument("--part-size-mb", type=int, default=32)

    # ✅ ускорение: DONE батчами
    ap.add_argument("--done-flush-every", type=int, default=200,
                    help="how many DONE ids to buffer before batch update (0=only at end of tar job)")
    ...
    # Timeweb creds из env
    dst_access = _env("TIMEWEB_ACCESS_KEY_ID")
    dst_secret = _env("TIMEWEB_SECRET_ACCESS_KEY")
    dst_token = _env("TIMEWEB_SESSION_TOKEN")

    if not dst_access or not dst_secret:
        raise SystemExit("Timeweb creds are required: set TIMEWEB_ACCESS_KEY_ID and TIMEWEB_SECRET_ACCESS_KEY env vars")
    ...
    src_cfg = S3SrcConfig(
        ...
        ignore_aws_env=args.src_ignore_aws_env,
    )
    dst_cfg = S3DstConfig(
    ...
    if args.once:
        run_once(
            ...
            done_flush_every=args.done_flush_every,
